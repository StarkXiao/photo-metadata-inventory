...
func TestParseTIFFFlagsNonCoordinateGPSMetadata(t *testing.T) {
	b := make([]byte, 48)
	copy(b, "II*\x00\x08\x00\x00\x00")
	b[8] = 1
	b[10], b[11] = 0x25, 0x88 // GPSInfoIFDPointer
	...
	b[18] = 30
	b[30] = 1
	// GPSAltitude (0x0006), without latitude/longitude, is still
	// location-related metadata and must be reported as a privacy risk.
	b[32], b[33] = 0x06, 0x00
	if _, _, gps, err := parseTIFF(b); err != nil || !gps {
		t.Fatalf("got gps=%v err=%v", gps, err)
	}
